    ...
    with chat_col:
        ...
        if not thought.thought_complete:
            ...
            if not session.continue_thought:
                ...
                st.error(
                    "Hackathon access to GPT-4 / stable-diffusion-xl "
                    "resources is no longer available; app is now in read-only mode"
                )
...
def render_thought_selection(brain: BrainV2, session: SessionData):
    ...
    st.button("Begin", on_click=_start_new, use_container_width=True, disabled=True)
    st.error(
        "Hackathon access to GPT-4 / stable-diffusion-xl resources is no longer available; app is now in read-only mode"
    )
...
def render_ai_output(brain: BrainV2):
    ...
    for idx, entry in enumerate(sorted_entries):
        ...
        if st.button("Load thought", key=f"gallery-load-{idx}"):
            brain.thought_memory.read_thought(entry.thought_id)
            # lift restriction on loading incomplete thoughts now that app is in read-only mode
            # if not thought.thought_complete:
