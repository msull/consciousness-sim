...
class GoalMemoryInterface(ABC):
    ...
    @abstractmethod
    def write_blog_entry(self, persona_name: str, title: str, content: str, thought_id: str) -> BlogEntry:
        pass
    ...
    def continue_thought(
        ...
        match step.tool_name:
            ...
            case prompts.ToolNames.WriteBlogPost:
                context, full_output = self._handle_write_blog_action(thought, step, _status_callback_handler)
            case _:
...
class BrainV2(BrainInterface):
    def _handle_write_blog_action(
        ...
        blog_entry_prompt = prompts.write_blog_entry(thought, persona, step, blog_title)
        blog_entry_content = get_completion(blog_entry_prompt)

        new_blog = self.goal_memory.write_blog_entry(
            persona_name=persona.name, title=blog_title, content=blog_entry_content, thought_id=thought.thought_id
        )
        ...
        return context, new_blog.format()
