...
from local_utils.brain import Brain, LearnThoughtResponse, ReflectThoughtResponse
from local_utils.brainv2 import BrainV2, MappingMemory, Thought
...
memory = get_memory()
BRAIN = BrainV2(memory=memory, logger=logger)
...
def main():
    ...
    session_data = SessionData.init_session(settings.session_data)
...
def render_main_functionality(settings: StreamlitAppSettings, session_data: SessionData):
    ...
    if not session_data.trigger_new_thought:
        ...
        with sidebar:
            with st.form("New thought"):
                st.selectbox("Thought model", ("gpt-3.5-turbo", "gpt-4"))
                if st.form_submit_button("Trigger new thought"):
                    thought = BRAIN.start_new_thought(force_id=session_data.session_id).model_dump()
                    session_data.thought = thought
...
def render_thought_nav_btns(session_data: SessionData, settings: StreamlitAppSettings):
...
def render_recent_thoughts(settings: StreamlitAppSettings, n=5):
    ...
    for session_file in sorted([x.name for x in settings.session_data.iterdir()], reverse=True):
        ...
        obj = SessionData.model_validate_json(session_text)
